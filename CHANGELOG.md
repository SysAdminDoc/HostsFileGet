...
- Wired the desktop global shortcuts and About dialog shortcut list to the registry, and documented `Ctrl+P`, `Ctrl+H`, and `Ctrl+/` alongside the existing save/search/refresh shortcuts.

**Release trust hardening**
- Added `scripts/build_release_artifacts.py` to compute the release EXE SHA-256, render Winget/Chocolatey manifests, create a reproducible package-manifest zip, and write `HostsFileGet.release-artifacts.json` in one repeatable step.
- Added `scripts/verify_release_artifact.py` to smoke-check a built EXE through `--version` and `--help` without launching the GUI.
- Updated CI/release workflows, release docs, package-manager docs, and release identity checks so checksum, package-manifest, EXE verification, PyInstaller advisory, and `pip-audit` gates stay in the release path.
...
- New `_HttpsOnlyRedirectHandler` + `safe_urlopen` wrap every outbound feed/whitelist/source-health/preview HTTP request in a `urllib.request.build_opener`-built opener that rejects redirects to anything other than `http`/`https`. Python's stock handler already blocks `file://` on modern releases, but the explicit allowlist also closes the residual `ftp://`-based SSRF path against internal services.
- `LocalApiRequestHandler` now sets `X-Content-Type-Options: nosniff`, `Referrer-Policy: no-referrer`, and `X-Frame-Options: DENY` on every response, matching the defensive header set the block-page server already carried.
- Six new regression tests cover the unicode case-fold path, regex-metachar-as-literal, literal replacement with backreference syntax, redirect rejection of `file://` / `ftp://`, and continued acceptance of `http`->`https` redirects. Total: 339 tests.
