...
Profile quick switching follows the same config-only boundary. HostsFileGet can switch saved app-config profiles from the GUI or optional tray menu, but the switch never writes the system hosts file and tray support stays an optional runtime dependency.
...
- Search, paged large-list removal, find/replace, adblock quarantine, context menu commands.
...
- PyInstaller packaging should be built from pinned dependencies and guarded by `scripts/check_release_identity.py`; the guard rejects PyInstaller pins below the safe range for `GHSA-p2xp-xx3r-mffc` and ensures the release workflow keeps the artifact-generation and EXE-verification scripts wired in.
