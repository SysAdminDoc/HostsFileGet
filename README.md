...
- Import pipeline:
  ...
  - manifest-only local source adapter plugins for reviewed external source packs
...
python hosts_editor.py --managed-package-export intune-win32 .\managed-hosts.txt .\managed-bundle --managed-package-version 2.26.0 --managed-installer-url https://github.com/SysAdminDoc/HostsFileGet/releases/download/v2.26.0/HostsFileGet.exe --managed-sha256 AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA --managed-label "Corp Managed"
```
...
Inspect manifest-only local source adapter plugins:

```powershell
python hosts_editor.py --source-adapter-list
...
Install and launch the optional `prompt_toolkit` TUI without changing the default GUI/CLI dependencies:
...
Preview or run the explicit loopback-only local block page server:
...
- Source adapter plugins: `docs/source-adapter-plugins.md`
